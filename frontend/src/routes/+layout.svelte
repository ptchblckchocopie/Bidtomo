...
{#if showDecorations}
  ...
  {:else if $backgroundStore === 'interactive_tile'}
    {#await import('$lib/components/SplineBackground.svelte') then { default: SplineBackground }}
      <SplineBackground />
    {/await}
  {:else if $backgroundStore === 'ripple_grid'}
    {#await import('$lib/components/RippleGridBackground.svelte') then { default: RippleGridBackground }}
      <RippleGridBackground />
    {/await}
  {:else if $backgroundStore === 'stream'}
    {#await import('$lib/components/SplineBackground.svelte') then { default: SplineBackground }}
      <SplineBackground url="https://prod.spline.design/3KAAuLj8dJzcMVPv/scene.splinecode" scale={100} offsetY={50} opacity={1} />
    {/await}
  {/if}
          ...
          <div class="bg-menu-container relative">
            ...
            {#if bgMenuOpen}
              <div class="bg-dropdown absolute left-1/2 -translate-x-1/2 mt-2 w-48 py-1 z-50">
                ...
                <button
                  onclick={() => selectBackground('ripple_grid')}
                  class="bg-dropdown-item {$backgroundStore === 'ripple_grid' ? 'active' : ''}"
                >
                  <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  ...
                  <span>Ripple Grid</span>
                </button>
                <button
                  onclick={() => selectBackground('stream')}
                  class="bg-dropdown-item {$backgroundStore === 'stream' ? 'active' : ''}"
                >
                  <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5.636 18.364a9 9 0 010-12.728m12.728 0a9 9 0 010 12.728M8.464 15.536a5 5 0 010-7.072m7.072 0a5 5 0 010 7.072M11 12a1 1 0 102 0 1 1 0 00-2 0z" />
                  </svg>
                  <span>Stream</span>
                </button>
                <button
