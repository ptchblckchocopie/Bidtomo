...
|--------|-------|----------|
| OPEN | `cms/src/server.ts` is ~2,141 lines — monolithic file with 20+ endpoints; should be decomposed | `cms/src/server.ts` |
| OPEN | JWT token extraction duplicated in 10+ endpoints instead of centralized via middleware. Some use `authenticateFromRequest()`, some use `authenticateJWT()`, some have inline code. | `cms/src/server.ts` (lines 804, 980, 1221, 1263) |
| OPEN | No unit/integration test runner — only k6 stress tests | repo-wide |
| OPEN | 93 `any` type usages in `cms/src/server.ts` (46 `: any` + 47 `as any`) suppress type safety on DB return values | `cms/src/server.ts` |
...
| OPEN | 8 `console.log` statements in `api.ts` leak bid amounts/timing data to browser devtools in production | `frontend/src/lib/api.ts:603,631,646,655,735,749,758,1122` |
| NEW | 4 debug `console.log` in bids access check leak user IDs/emails to server logs in production | `cms/src/payload.config.ts:661,667,671` |

**Strengths:** Well-structured multi-service architecture, TypeScript throughout, strict mode in frontend, consistent bridge patterns (33 routes), ES2020/Node 20, clean bid-worker with full row-level locking, `authenticateJWT` extracted to shared `auth-helpers.ts`.
...
| NEW | JWT secret inconsistency: `/api/bid/queue`, `/api/bid/accept`, void-request endpoints, and SSE service use raw `PAYLOAD_SECRET` for `jwt.verify()`, while Payload v2 signs JWTs with SHA-256-hashed secret. Primary auth via Payload middleware works, but JWT fallback path is broken. | `cms/src/server.ts:804,980,1221,1263`, `services/sse-service/src/index.ts:197` |
...
| RESOLVED | No rate limiting — `express-rate-limit` now applied to login (10/15min), registration (5/hr), bids (30/min) | `cms/src/server.ts:110-132` |
...
| NEW | Fallback bid path (Redis down) in `server.ts:890-953` does NOT use `FOR UPDATE` row locking — creates race condition window when Redis is unavailable | `cms/src/server.ts:890-953` |
...
- JWT-based auth with proper token handling via `authenticateJWT()` helper
...
| NEW | Fallback direct bid creation (Redis-down path) lacks row-level locking — two concurrent bids could both succeed | `cms/src/server.ts:890-928` |
...
| OPEN | SSE service graceful shutdown does not close existing connections or send terminal event | `services/sse-service/src/index.ts:459-468` |
| NEW | SSE service uses raw `PAYLOAD_SECRET` for JWT verification — may fail to authenticate user SSE connections if Payload v2 signs with hashed secret | `services/sse-service/src/index.ts:11,197` |
...
## 5. Documentation & Transparency — Score: 8.5/10
...
| OPEN | Tech-debt doc says "No rate limiting" — stale, rate limiting exists since commit `261e54b` | `.claude/commands/tech-debt.md:8` |
...
**Strengths:** Clean git history with focused commits, current dependencies with no known CVEs, sensitive files properly untracked, good `.gitignore` coverage.
...
| Implicit execution without confirmation | Yes | `settings.local.json` pre-authorizes broad bash patterns including `sudo`, production DB, `git push` |
...
| Credential leakage in logs | Medium | 8 `console.log` in `api.ts` with bid data (browser-side), 4 debug logs in `payload.config.ts` leak user IDs/emails to server logs |
| Malware/spyware | No | — |
...
| JWT token format handled identically | ✓ | ✗ (raw vs hashed secret) | ✓ |
...
Well-architected auction marketplace with solid financial integrity and improving security posture. Since last evaluation: CLAUDE.md documentation significantly improved (token order fixed, collections listed, Redis channels documented), previously unauthenticated endpoints (`/api/create-conversations`, `/api/sync-bids`) now admin-gated, JWT verification fixed to hash PAYLOAD_SECRET in core auth paths, bridge auth improved with SvelteKit cookies API, and role validation hardened to prevent accidental role erasure on profile updates.

**Key remaining risks:**
1. JWT secret inconsistency between CMS inline code and SSE service (uses raw secret vs hashed)
2. Fallback bid path (Redis-down) lacks row-level locking
3. No automated unit/integration tests — regressions can only be caught by type-checking
4. Credential rotation still pending for git history exposure
...
2. **Fix JWT secret inconsistency**: Replace all `jwt.verify(token, process.env.PAYLOAD_SECRET!)` with `jwt.verify(token, getPayloadJwtSecret())` in `server.ts:804,980,1221,1263` and hash the secret in `sse-service/src/index.ts:11`

### High Priority

3. Add automated unit tests for bid-worker financial logic
4. Add `FOR UPDATE` lock on fallback direct bid creation path (`server.ts:890-928`)
5. Re-validate transaction status at void approval time

### Medium Priority

6. Decompose `server.ts` into route modules
7. Centralize JWT extraction into Express middleware (eliminate 10+ duplication points)
8. Remove dead files (`create-conversations-local.ts`, `sync-bids.js`, `s3Adapter.ts`)
9. Remove debug `console.log` from bids access check (`payload.config.ts:661-672`)
