...
- **ES search route must NOT be `/api/products/search`** — Payload's built-in `GET /api/products/:id` intercepts it (treats "search" as an id → NaN → 500). The correct route is `GET /api/search/products`.
- **ES v9 client `media_type_header_exception`** — The `@elastic/elasticsearch` v9 client sends `compatible-with=9` vendored headers. v7/v8 ES servers reject these. Fix: override headers in the Client constructor with plain `application/json` (already done in `elasticSearch.ts`).
- **Staging DB requires migrations before CMS starts** — A fresh Railway staging Postgres has no tables. Run `payload migrate` (with public Postgres URL since `*.railway.internal` DNS doesn't resolve locally) before the CMS will work.
